...
def summarize_interaction_metrics(
    ...
    grounded = [row for row in normalized if row["has_grounded"]]
    non_grounded = [row for row in normalized if not row["has_grounded"]]
    ...
    precise_ask_tasks = sum(
        ...
        if row["has_grounded"] and row["answered_ask_count"] > 0
    )
