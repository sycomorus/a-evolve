...
def print_evolve_summary(summary: dict[str, Any], *, console: Console | None = None) -> None:
    ...
    table.add_row("Epochs", str(summary.get("epochs_completed", summary.get("max_epochs", 0))))
    table.add_row("Updates", str(summary.get("updates_completed", summary.get("cycles_completed", 0))))
    table.add_row("Train tasks", str(summary.get("train_limit", "")))
    table.add_row("Batch size", str(summary.get("batch_size", "")))
